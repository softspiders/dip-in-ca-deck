...
Очевидно, что во время выполнения нашего примера необходимо, чтобы из модуля бизнес-логики вызывался логгер.
Понятно, что мы имеем неизбежную run-time-зависимость бизнес-логики от логгера.

Отметим, что design-time-зависимость между модулями, как правило, означает, что между ними присутствует и
run-time-зависимость, но, как дальше будет показано, обратное неверно. В дальнейшем для краткости, если не будет
специальных уточнений, слово "зависимость" будем означать именно "design-time-зависимость".   
...
Как ранее уже было отмечено, зависимости создают множество проблемы, которые необходимо так или иначе решать
(по-английски решение проблем этого рода называется одним ёмким словом - *decoupling*).
...
Понятие интерфейса является, наверное, одним из наиболее значимых в программировании. Сегодня оно широко известно, не
