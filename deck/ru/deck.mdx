...
Кто-то наверняка скажет или подумает, что некоторые детали предлагаемой "разжёвываются" больше, чем нужно.
...
Требуемый интерфейс не обязательно должен принадлежать *вызывающему* модулю. Он может быть внешним, например,
определяться каким-то стандартом.  
В таких случаях говорят, что оба модуля - вызывающий и реализующий - зависят от одной и той же абстракции.   
