...
<Step subtitle="Наивная реализация журналирования">

```ts file=../src/naive/NaiveCounter.ts title="NaiveCounter.ts"
```

```ts file=../src/naive/NaiveLogger.ts title="NaiveLogger.ts"
```

</Step>

<Step subtitle="Design-time-зависимость от реализации">

```ts 1,5 file=../src/naive/NaiveCounter.ts title="NaiveCounter.ts"
```

```ts 1 file=../src/naive/NaiveLogger.ts title="NaiveLogger.ts"
```
...
<Step subtitle="Реализация с использованием DIP">

```ts file=../src/better/CounterInteractor.ts title="CounterInteractor"
```

```ts file=../src/better/LoggerImpl.ts title="LoggerImpl"
```
...
```ts 1,3 file=../src/better/LoggerImpl.ts title="LoggerImpl"
```
